       ...
       <v-list subheader>
         ...
         <v-list-tile avatar v-for="item in myFriends" :key="item.en_name" @click="selectFriend(item.id, item.chatroute)">
           ...
           <v-list-tile-action>
             ...
             <span slot="badge">{{item.unread}}</span>
              </v-badge>
           </v-list-tile-action>
         </v-list-tile>
       </v-list>
...
vm.connected = false
for(var i =0; i<data.myClients.length; i++){

  if(data.myClients[i] == vm.$socket.id){

      vm.connected = true
    vm.myMessages.push({'avatar' : 'https://scontent.ficn2-1.fna.fbcdn.net/v/t1.0-1/p160x160/29468236_901369833374211_8734349036217171968_n.jpg?_nc_cat=0&oh=f8f7428a3e9e807d58b3ef91ef215062&oe=5B760837', 'name': vm.secondUserName, 'message': data.myMessage})
      vm.scrollToEnd()
  }
...
if(!vm.connected){
  ...
  console.log('what'vm.myFriends[data.myId])
    ...
    console.log('myClients', data.myClients, 'mySocket', data.mySocket)
...
vm.tempMessage = vm.message
  ...
  vm.$socket.emit('latestMessage', {myMessage: vm.message, myId: vm.currentUserId, mySocket: vm.$socket.id})
